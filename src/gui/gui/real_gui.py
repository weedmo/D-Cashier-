...
JSON_PATH = '/home/joonmo/D-Cashier-/src/gui/resource/product_data.json'
...
class CashierGUI(Node):
    def __init__(self):
        ...
        # ROS 2 Subscribers
        self.create_subscription(Bool, "/prompt_pub", self.prompt_callback, 10)
        self.create_subscription(String, "/class_name", self.class_name_callback, 10)
        self.create_subscription(Bool, "/wakeup_detected", self.wakeup_callback, 10)
        self.create_subscription(Int32, "/adult_check_result", self.adult_result_callback, 10)
    ...
    def class_name_callback(self, msg):
        name = msg.data
        price = self.product_data.get(name, None)

        if price is None:
            display = f"⚠️ '{name}'의 가격 정보가 없습니다."
        elif name == "terra":
            display = f"🅾️ 성인 인증이 필요합니다 🅾️\n(신부증을 준비해주세요)"
            self.waiting_adult_item = True
            self.speak("성인 인증이 필요합니다. 신부증을 제시해주세요", voice="nova")
        else:
            self.cart_items.append((name, price))
            self.total_price += price
            display = f"📦 최근 인식: {name} ({price}원)\n\n🧾 장바구니:\n"
