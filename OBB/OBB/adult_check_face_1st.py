...
class IDVerificationNode(Node):
    ...
    def verify_callback(self, request, response):
        ...
        # 신분증 OCR 단계
        ocr_success = self.run_id_ocr_stage()
        if not ocr_success:
            self.get_logger().warn("⚠️ OCR 단계 실패. 미성년자 또는 사용자 취소.")
        ...
        if not face_success:
            self.get_logger().warn("⚠️ 얼굴 인증 실패 또는 사용자 취소.")
            self._release_resources()
            return response
    ...
    def run_id_ocr_stage(self):
        ...
        while True:
            ...
            if score < 0.68:
                ...
                if hold_start_time is None:
                    ...
                    if not (birth_num and code):
                        ...
                        hold_start_time = None  # 다시 대기 초기화
                        continue
                    if self.is_adult(birth_num, code):
                        self.get_logger().info("✅  성인입니다. 얼굴 인식을 위해 준비할 시간을 제공합니다.")
                        time.sleep(3)  # 사용자에게 얼굴 준비 시간 제공
                        return True
                    else:
            ...
            if cv2.waitKey(1) == 27:
                self.get_logger().warn("⛔️ 사용자에 의해 OCR 단계가 취소되었습니다.")
                cv2.destroyAllWindows()
                return False
